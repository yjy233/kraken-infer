...
- native Qwen3.5 VL mixed prefill 已能把 CPU vision encoder 产出的 image embeddings
  和文本 embeddings 喂给同一个 Metal decoder；但 MTP 只保留在纯文本请求上，
  图片请求会返回 `X-Kraken-MTP-Disabled-Reason:
  multimodal_prompt_not_supported_with_mtp`。
...
- 多模态 prompt 与 MTP 暂不同时启用；VL 请求保留 native mixed prefill，但走
  no-MTP decode。
...
- image embedding row 会进入 target prefill，并导出普通 decode 需要的 hidden/cache。
- physical KV position 与 logical MRoPE position 已分离，图片 grid 按 Qwen3.5
  VL 规则推进 logical position。
- MTP GGUF + mmproj 图片请求会禁用 MTP；`make qwen35-vl-mtp-test` 会检查
...
{"mtp": {"x-kraken-mtp-enabled": "0", "x-kraken-mtp-disabled-reason": "multimodal_prompt_not_supported_with_mtp", ...}}
```
...
| VL tiny PNG, `max_tokens=2` | 约 10.58s | 约 10.66s | 回退前测量；CPU vision + prefill 主导，MTP 只 draft 1 个 token |
...
后续基于 release 性能对比，文本 MTP 已能取得正收益，但 VL MTP 在 tiny image
短输出上收益不稳定，并且真实大图主要受 CPU vision prefill 限制。因此当前实现把
MTP 限定为纯文本请求；VL 请求即使使用 MTP GGUF 也会明确回退为 no-MTP。
...
- `scripts/test_qwen35_vl_gateway.py --expect-mtp-disabled-reason
  multimodal_prompt_not_supported_with_mtp` 可验证 MTP GGUF + mmproj 图片请求走
  native VL mixed prefill，并明确禁用 MTP。
