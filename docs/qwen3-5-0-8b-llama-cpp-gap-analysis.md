...
- 使用带 MTP 的 GGUF 时，图片请求仍会走 native VL mixed prefill，但 MTP 被显式
  禁用，原因是 `multimodal_prompt_not_supported_with_mtp`。当前只保留文本 MTP。
...
- MTP `draft_tokens=3, p_min=0`：`real 14.38s`，
  drafted=62、accepted=28、verify_steps=50、adaptive_budget=1、adaptive_changes=4。
...
- tiny PNG VL `max_tokens=2`：no-MTP 约 `10.58s`，MTP 约 `10.66s`；
  CPU vision + prefill 主导。基于这组结果，VL MTP 已回退为 disabled，当前只保留
  文本 MTP。
...
- 多模态 prompt 与 MTP 的 cache/process 语义暂不合并；图片请求明确走 no-MTP。
